...
---
# Surface Hub での Windows 更新プログラムの管理
...
業務時間中は常にデバイスを使用できるように、Surface Hub では、指定したメンテナンス期間中に管理機能を実行します。 メンテナンス期間中に、Surface Hub は自動的に Windows Update または WSUS から更新プログラムをインストールし、必要に応じてデバイスを再起動します。
